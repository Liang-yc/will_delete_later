    ...
    def __init__(self, args):
        ...
        if args.auto_augment:
            self.policies = [
                ...
                # ['randomErazer',0.5,9,'randomErazer',0.5,1]
            ]
    ...
    def flow(self, x, y=None, batch_size=32, shuffle=True, sample_weight=None,
        ...
        while True:
            ...
            if self.args.auto_augment:
                ...
                lamda = np.random.rand(x_batch.shape[0])
                for i in range(x_batch.shape[0]):
                    x_batch[i] = apply_policy(x_batch[i], self.policies[random.randrange(len(self.policies))])
                    # id = np.random.randint(0,x_batch.shape[0]-1)
                    # x_batch[i] = lamda[i]*x_batch[i]+(1-lamda[i])*(x_batch[id])
                    # y_batch[i] = lamda[i]*y_batch[i]+(1-lamda[i])*(y_batch[id])
